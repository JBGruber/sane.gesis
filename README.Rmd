...
Use `plan_local_repo()` to scan your project directory and find all packages that you have used in a project (and their dependencies):

```{r example-plan, warning=FALSE}
library(sane.gesis)
plan_local_repo(".")
...
They can then build a local repository using `build_local_repo()`:

```{r example-build, warning=FALSE}
plan_local_repo(".") |>
  build_local_repo()
